...
import {
  ...
  DollarSign, Video, Phone, MessageCircle, MapPin,
  Star, Shield, Calendar, Sparkles, CheckCircle
} from 'lucide-react'
...
const QUIZ_STEPS = [
  {
    ...
    multiSelect: true,
    options: [
      { id: 'anxiety', label: 'Anxiety & Stress', emoji: '😰', color: '#3a6d77' },
      { id: 'depression', label: 'Depression', emoji: '🌧️', color: '#7c5cbf' },
      { id: 'trauma', label: 'Trauma / PTSD', emoji: '💔', color: '#dc2626' },
      { id: 'relationships', label: 'Relationships', emoji: '💑', color: '#c4b5e2' },
      { id: 'transitions', label: 'Life Transitions', emoji: '🔄', color: '#b8a88f' },
      { id: 'identity', label: 'Identity & Self-Exploration', emoji: '🪞', color: '#16a34a' },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { id: 'traditional', label: 'Traditional Talk Therapy', desc: 'CBT, DBT, psychodynamic approaches', icon: Brain },
      { id: 'holistic', label: 'Holistic / Integrative', desc: 'Mind-body-spirit connection, somatic practices', icon: Heart },
      { id: 'mindbody', label: 'Mind-Body Practices', desc: 'Yoga therapy, breathwork, somatic experiencing', icon: Sparkles },
      { id: 'spiritual', label: 'Spiritual Counseling', desc: 'Meaning-making, existential exploration', icon: Users },
    ],
  },
  {
    id: 'sessionType',
    title: 'What session format works best for you?',
    subtitle: 'Select all that you're open to',
    multiSelect: true,
    options: [
      { id: 'video', label: 'Video Sessions', desc: 'Live face-to-face over video', icon: Video },
      { id: 'phone', label: 'Phone Sessions', desc: 'Audio-only calls', icon: Phone },
      { id: 'chat', label: 'Text / Messaging', desc: 'Asynchronous written therapy', icon: MessageCircle },
      { id: 'inperson', label: 'In-Person (if available)', desc: 'Traditional office visits', icon: MapPin },
    ],
  },
  {
    id: 'budget',
    title: 'What's your budget range per session?',
    subtitle: 'Select the range that fits your situation',
    multiSelect: false,
    options: [
      { id: 'low', label: '$0 – $50', desc: 'Looking for affordable or sliding scale options', icon: DollarSign },
      { id: 'mid', label: '$50 – $100', desc: 'Standard therapy session rates', icon: DollarSign },
      { id: 'high', label: '$100 – $150', desc: 'Premium services', icon: DollarSign },
      { id: 'premium', label: '$150+', desc: 'Specialized or high-touch care', icon: DollarSign },
    ],
  },
  {
    id: 'preferences',
...
// ─── Quiz Option Card ─────────────────────────────────────────────────────────
function QuizOption({ option, selected, onToggle, multiSelect }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: selected ? `0 4px 20px ${option.color || P.green}25` : '0 2px 8px rgba(0,0,0,0.04)',
      }}
    >
      {/* Checkmark indicator */}
      {selected && (
      ...
      <div className="flex items-start gap-3">
        ...
        {hasIcon ? (
          ...
          >
            <Icon className="w-5 h-5" style={{ color: option.color || P.sage }} />
          </div>
        ) : hasEmoji ? (
      ...
      <div className="p-6">
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${therapist.color}, ${therapist.color}cc)` }}
          >
            {therapist.photo}
          </div>

          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-base mb-0.5 leading-snug" style={{ color: P.charcoal }}>
              {therapist.name}
            </h3>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-bold" style={{ color: P.charcoal }}>{therapist.rating}</span>
              </div>
        ...
        {/* Bio */}
        <p className="text-sm leading-relaxed mb-4" style={{ color: '#6b7b6a' }}>
          {therapist.bio}
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {therapist.tags.map(tag => (
            <span
              ...
              style={{ background: `${therapist.color}15`, color: therapist.color }}
            >
              {tag}
            </span>
          ))}
        ...
        {/* Meta row */}
        <div className="flex items-center justify-between mb-5 text-xs" style={{ color: P.sage }}>
...
export default function TherapistFinder() {
  ...
  const canProceed = () => {
    ...
    if (step.id === 'preferences') return true // Optional step
  ...
  const matchedTherapists = showResults
    ? THERAPISTS.map(t => {
        let score = 0
        const concernsMatch = (answers.concerns || []).some(c => t.concerns.includes(c))
        const approachMatch = t.approaches.includes(answers.approach)
        const sessionMatch = (answers.sessionType || []).some(s => t.sessionTypes.includes(s))
        const budgetFits = (() => {
        ...
        const prefsMatch = (answers.preferences || []).every(p => t.preferences.includes(p))
  ...
  if (showResults) {
    return (
      <div className="min-h-screen py-16" style={{ background: 'linear-gradient(180deg, #f0f7f0 0%, #fdfcf7 100%)' }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            ...
            >
              ...
              <span className="text-sm font-bold" style={{ color: P.green }}>Quiz Complete</span>
            </div>

            <h1 className="text-3xl sm:text-4xl font-extrabold mb-4" style={{ color: P.charcoal }}>
              We found {matchedTherapists.length} great matches for you 🌿
            </h1>
            <p className="max-w-xl mx-auto text-base mb-6" style={{ color: P.sage }}>
              Based on your preferences, here are therapists who align with your needs and goals.
              All are licensed, vetted, and ready to support your journey.
            </p>
          ...
          {matchedTherapists.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sm" style={{ color: P.sage }}>
                ...
                <button onClick={handleReset} className="underline font-semibold" style={{ color: P.green }}>
                  retake the quiz
                </button>
                .
              </p>
          ...
          <div className="mt-16 rounded-3xl overflow-hidden">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${P.teal}, ${P.green})` }}
            >
              <h3 className="text-white font-bold text-xl mb-2">
                Not sure which therapist is right?
              </h3>
              <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.8)' }}>
                Our intake coordinators can help you choose based on a brief phone call.
              </p>
              <Link
                to="/contact"
  ...
  // ── Quiz View ─────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen py-16" style={{ background: 'linear-gradient(180deg, #fdfcf7 0%, #f0f7f0 100%)' }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold uppercase tracking-wider" style={{ color: P.sage }}>
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ background: '#e8e4dd' }}>
            <div
        ...
        <div
          ...
          style={{ background: 'white', border: '1.5px solid #f0ede8', boxShadow: '0 4px 24px rgba(51,54,69,0.08)' }}
        >
          {/* Question header */}
          ...
          {/* Helper text */}
          {step.multiSelect && (
            <p className="text-center text-xs mt-5" style={{ color: '#c4bdb3' }}>
        ...
        <button
          ...
          style={{ background: `linear-gradient(135deg, ${P.green}, ${P.teal})` }}
        >
        ...
        <p className="text-center text-xs mt-6" style={{ color: '#c4bdb3' }}>
          🔒 Your responses are private and only used to match you with the right therapist
        </p>
      </div>
    </div>
  )
